...
API_IMPL_BEGIN
  ...
  if (options) {
    ...
    #if 0
    auto providers = options->provider_factories;
      const IExecutionProvider* cpu_provider = providers.Get(onnxruntime::kDmlExecutionProvider);
  ...
  const ExecutionProviders& providers = session_state_->GetExecutionProviders();
  const IExecutionProvider* cpu_provider = providers.Get(onnxruntime::kDmlExecutionProvider);
  auto ep = options->provider_factories.
  Dml::GetDmlDevice(dmlDevice);
#endif
